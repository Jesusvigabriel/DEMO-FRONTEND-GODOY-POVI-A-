<template>
    <v-container>
        ...
        <v-row v-show="!tieneLOTE && !tienePART">
        ...
        <v-row v-show="tienePART">
            <v-col class="py-0" v-if="items.length>0">
                <v-data-table v-if="!this.textil"
                    :headers="cabecerasListaPART" 
                    :items="items"  
                    :footer-props="{itemsPerPageOptions:[10,30,100,-1]}"   
                    :items-per-page="30" 
                    :search="textoBusqueda"
        ...
        <v-dialog
            ...
            persistent
        >
            <v-card v-show="!tieneLOTE && !tienePART">
                <v-card-title>
    ...
    data() {
        return {
            ...
            cabeceras: [
                    ...
                    {text: 'Peso', align: 'end', value: 'Peso'},
    ...
    methods: {
        ...
        seleccionarItem(item) {
            if(this.tienePART){
                ...
                this.partidaLeida=item.Partida
                this.BarcodeLeido=item.IdProducto
      ...
      procesarIngresos() {
        ...
        async popularListaProductos() {
            if(this.tieneLOTE){
                ...
                productos.getAllPartidas(this.idEmpresa)
                .then(respuesta => {
                    respuesta.forEach(e => {
        ...
        crearOrdenConfirmado() {
            ...
            this.items.forEach(unItem => {
                if(this.tieneLOTE){
                    ...
                    detalleDeLaOrden.push({cantidad: unItem.Cantidad, importe: unItem.Importe, barcode: unItem.Barcode, partida: unItem.Partida, idProducto: unItem.IdProducto, idPartida: unItem.Id})
                }else{
        ...
        procesarBarcode() {
            if(this.loteCompleto){
                ...
                if(this.tieneLOTE){
                    ...
                    productos.getProductoByPartidaAndEmpresaAndProductov2(this.idEmpresa,this.partidaLeida,this.BarcodeLeido)
                    .then(response=>{
                        this.articuloAProcesar=response[0]
                        if (!this.textil) {
                            this.$nextTick(() => {this.$refs.CantidadAIngresar.focus()})
                        }
                    })
        ...
        procesarCantidad() {
            if(this.tieneLOTE){
                ...
                if (this.CantidadAIngresar>0) {
                    ...
                    let sumaCantidad = 0
                    let barcode = this.articuloAProcesar.Barcode
                    let partida = this.articuloAProcesar.Partida
                    let cantidad = this.CantidadAIngresar
    
                    this.items.forEach ( item => {
                        if ( barcode === item.Barcode && partida === this.articuloAProcesar.Partida) {
                            sumaCantidad = parseInt(item.Cantidad) + parseInt(cantidad)
                            if(sumaCantidad <= item.StockDisponible){
                                item.Cantidad += parseInt(cantidad)
                                item.Importe += parseFloat(this.ImporteAIngresar)
